...
Due to missing values, the mean has been reduced to about `r round(mean.total.per.day.1, 0)` steps per day. The median is slightly higher at `r median.total.per.day.1` steps per day.
...
The graph shows an interesting pattern where the subject seems to be active at two time points daily for the first month and then at one time point in the better part of the second month. On average, the time of day that the subject is most active is around `r data[data$interval == max.interval, "time"][1]`. 
...
From the computation, missing data accounts for about `r round(nMissing/nrow(data) * 100, 2)` percent of the total dataset. This is quite a substantial amount and it's impact on estimates of the total daily number of steps is assessed by first filling the missing values with the average of their interval and then fitting a histogram.

```{r filling.missing.values, fig.height = 9}
...
Missing values on average affects the mean by reducing it. In this case the mean was pulled down to `r round(mean.total.per.day.1, 0)` steps by the missing values. When they were filled, the vaues brought it closer to the median, that is, the mean of `r round(mean.total.per.day.2, 0)` steps with the median being `r median.total.per.day.1` steps. The median is never affected as it takes the position of the values rather than the individual values. 
...
```{r activity.patterns.weekdays.weekend}
...
Interestingly, the subject seems to be more active over the weekends than on the weekdays. Over the weekend, the subject has fluctuaing activity levels between `r data2[data2$interval == 600, "time"][1]` and about `r data2[data2$interval == 1900, "time"][1]`. However, during the weekdays, the subject seems to have a consistent time period of activity. The first period is in the morning between `r data2[data2$interval == 600, "time"][1]` and about `r data2[data2$interval == 940, "time"][1]`. The second period is in the late afternoon/early evening between `r data2[data2$interval == 1700, "time"][1]` and about `r data2[data2$interval == 1900, "time"][1]`. 

## Conclusion

On average, the subject seems to be 'active' with about `r median.total.per.day.1` steps. This is according to [a pubmed publication](http://www.ncbi.nlm.nih.gov/pubmed/14715035), where they suggest a classification of pedometer reading. However, in asmuch as the review suggests an active lifestyle, the data also shows sporadic difference not only between different time intervals but also between weekdays and weekends. Overall, classification of the steps taken would depend on the subject's underlining reason for using it and their ultimate goal.
